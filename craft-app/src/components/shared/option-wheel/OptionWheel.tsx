...
export default function OptionWheel({
  ...
  useEffect(() => {
    ...
    if (selectedIndexProp === selectedRef.current) return;
    applyTarget(selectedIndexProp, true);
  }, [selectedIndexProp, applyTarget]);
  ...
  return (
    <div
      ...
      className={cn(
        "relative w-full h-full overflow-hidden select-none touch-none outline-none",
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={cn(
            "absolute top-1/2 whitespace-nowrap leading-none will-change-[transform,opacity,filter] cursor-pointer",
